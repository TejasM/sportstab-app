...
public class Sprite implements java.io.Serializable {
	...
	public void setHided(boolean hided) {
		this.hided = hided;
		if (this.hasBall) this.ball.setHided(hided);
	}
	...
	public void paintName() {
		...
		if (!name.equals("don't")){
			...
			c.drawBitmap(bmp, bmp.getWidth() / 2, bmp.getWidth() / 2, null);
		}
	}
	...
	public boolean isTouched(float x2, float y2) {
		if (((this.isButton)&&(this.state==BLOCK))||(hided==true)){
			return false;
		}else{
			float dX = this.x - x2;
			float dY = this.y - y2;
			double curDist = Math.sqrt(dX * dX + dY * dY);
			return curDist < (this.r * 1.5);//Jackson: the factor is just to make the touchbox a lil bigger, i might want to adjust according to different devices.
		}
	}
